...
import com.pmob.pencarianrs.utils.FetchFromServerTask;
import com.pmob.pencarianrs.utils.FetchFromServerUser;
import com.pmob.pencarianrs.utils.GetLocation;
import com.pmob.pencarianrs.utils.JSONParser;
import com.pmob.pencarianrs.utils.PlaceBean;
...
public class LokasiActivity extends AppCompatActivity implements FetchFromServerUser {
...
//                    markerPlace.icon(BitmapDescriptorFactory.fromBitmap(bitmap));
                    ...
                    googleMap.animateCamera(CameraUpdateFactory.newCameraPosition(new CameraPosition.Builder().target(new LatLng(getMarkerLat(), getMarkerLong())).zoom(14.0f).build()));
                    int i = posPlace;
                    posPlace++;
...
//                            iconPlace.setImageBitmap(DownloadImage(placeIconMap.get(marker.getId())));
                            Picasso.get().load(placeIconMap.get(marker.getId()))
